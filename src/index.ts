  ...
  if (authState._ !== 'authStateSignedIn'/*  || 1 === 1 */) {
    ...
    switch (authState._) {
      case 'authStateSignIn':
        pagePromise = (await import('./pages/pageSignIn')).default.mount(); // Логин
        break;
      case 'authStateSignQr':
        pagePromise = (await import('./pages/pageSignQR')).default.mount(); // Логин по qr
        break;
      case 'authStateAuthCode':
        pagePromise = (await import('./pages/pageAuthCode')).default.mount(authState.sentCode);  // Получить код
        break;
      case 'authStatePassword':
        pagePromise = (await import('./pages/pagePassword')).default.mount(); // Ввести двухфакторний пароль 
        break;
      case 'authStateSignUp':
        pagePromise = (await import('./pages/pageSignUp')).default.mount(authState.authCode); // Получить код для регистрации
        break;
      case 'authStateSignImport':
        pagePromise = (await import('./pages/pageSignImport')).default.mount(authState.data); // Получить залогиненную страницу
        break;
      case 'authStateSignImportClient':
        pagePromise = (await import('./pages/pageSignImport')).default.mount(authState.data); // Получить страницу с активным логином
        break;
    }
    // });
